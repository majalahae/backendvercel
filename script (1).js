...
function addTitleText(title){
  ...
  titleText = new fabric.Textbox(title, {
    left:70, top:930, width:960, fontSize:100, fill:"#ffffff",
...
async function generatePoster(){
  ...
  try{
    const res = await fetch("https://backendvercel-fawn.vercel.app/api/generate", {
      method:"POST", headers:{"Content-Type":"application/json"}, body:JSON.stringify({url:inputUrl})
    });
    ...
    if(data.image_base64){
      fabric.Image.fromURL(data.image_base64, img=>{
        ...
        loadTemplateOverlay(getRandomOverlay()); addTitleText(data.title);
      });
    } else { addTitleText(data.title); loadTemplateOverlay(getRandomOverlay()); }
  } catch(e){ console.error(e); alert("Terjadi kesalahan saat generate poster!"); }
}
...
document.getElementById("copyCaptionBtn").onclick = ()=>{
  const caption = document.getElementById("captionBox");
  caption.select(); caption.setSelectionRange(0,99999);
  navigator.clipboard.writeText(caption.value); alert("Caption copied!");
};
...
function initIridescence({selector="#iridescence-container", color=[1,1,1], speed=1.0, amplitude=0.1, mouseReact=true}={}){
  ...
  const program=new OGL.Program(gl,{vertex:vertexShader,fragment:fragmentShader,uniforms:{
    uTime:{value:0},
  ...
  function animate(t){ program.uniforms.uTime.value=t*0.001; renderer.render({scene:mesh,camera:null}); requestAnimationFrame(animate);}
...
window.addEventListener("DOMContentLoaded",()=>{ initIridescence({selector:"#iridescence-container", color:[1,1,1], speed:1.0, amplitude:0.14, mouseReact:true}); });
